...
static void read_mouse (void) 
{
  ...
  if(currprefs.jports[0].id == JSEM_MICE + 1 || currprefs.jports[1].id == JSEM_MICE + 1) {
    // dPad is mouse
  	unsigned char *keystate = NULL;//SDL_GetKeyState(NULL);
...
/*
  ...
  nr_joysticks = 1;SDL_NumJoysticks ();
...
extern unsigned char uae4all_keystate[320];
...
static void read_joystick (void)
{
  ...
  {
  	unsigned char *keystate = uae4all_keystate;//SDL_GetKeyState(NULL);
    ...
    if(keystate[RETROK_LALT]) { // Pandora Start button
